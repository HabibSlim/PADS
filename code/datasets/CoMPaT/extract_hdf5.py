...
def write_hdf5(split, hdf5_dir, semantic_level, zips_path, meta_dir, n_compositions):
    data_loader = StylizedShapeLoader(
        zip_path=zips_path,
        meta_dir=meta_dir,
    ...
    if open_mode == "w":
        train_hdf5.create_dataset(
            "points", shape=(n_samples, N_POINTS, 3 + 3), dtype="float32"
        )
        train_hdf5.create_dataset(
            "points_part_labels", shape=(n_samples, N_POINTS), dtype="uint16"
        )
        train_hdf5.create_dataset(
            "points_mat_labels", shape=(n_samples, N_POINTS), dtype="uint8"
        )
        train_hdf5.create_dataset("shape_id", shape=(n_samples), dtype="S6")
        train_hdf5.create_dataset("style_id", shape=(n_samples), dtype="S6")
        train_hdf5.create_dataset("shape_label", shape=(n_samples), dtype="uint8")
...
def main(args):
    ...
    print(
        "[%s_%s_%d] Done!" % (args.split, str(args.semantic_level), args.n_compositions)
    )
