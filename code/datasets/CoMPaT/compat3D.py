...
import numpy as np
import trimesh
import datasets.CoMPaT.utils3D.gltf as gltf
import datasets.CoMPaT.utils3D.pointcloud as pc
from datasets.CoMPaT.utils3D.semantic_levels import SemanticLevel, open_meta
...
class ShapeLoader(CompatLoader3D):
    ...
    def _list_models(self, split_models):
        ...
        if self.filter_class != []:
            # Get indices of shapes with label in filter_class
            idx = np.where(np.isin(self.shape_labels, self.filter_class))[0]
            # Only return shapes with label in filter_class
            self.shape_ids = [self.shape_ids[i] for i in idx]
            self.shape_labels = [self.shape_labels[i] for i in idx]
