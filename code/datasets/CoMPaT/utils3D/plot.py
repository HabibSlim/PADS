...
def concat_figs(ims, pic_height):
    """
    ...
    # Resize so that height is 225 pix
    new_width = int(new_im.size[0] * pic_height / new_im.size[1])
    new_im = new_im.resize((new_width, pic_height), Image.BILINEAR)
...
def plot_pointclouds(
    ...
    def set_ticks(d, ax, p):
        fn = {
            0: (ax.set_xticks, ax.set_xticklabels),
    ...
    def make_fig(k, p, size, color=None, use_cmap=False):
        ...
        ax.get_proj = lambda: np.dot(Axes3D.get_proj(ax), np.diag([x_d, 1.0, z_d, 1.0]))

        if color is None:
            ax.scatter(p[:, 0], p[:, 1], p[:, 2], s=point_size, alpha=0.8)
        else:
            if use_cmap:
                ax.scatter(
                    p[:, 0],
                ...
                ax.scatter(p[:, 0], p[:, 1], p[:, 2], s=point_size, c=color, alpha=0.8)
        ax.dist = 30
    ...
    return concat_figs(all_ims, pic_height)
