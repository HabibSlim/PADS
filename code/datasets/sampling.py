...
def sample_sphere(num_points, bounding_radii=1.):
    """
...
"""
def _unbalanced_volume_sampling(mesh, num_points, radius, hash_resolution, contain_method, get_occ=True):
...
def _unbalanced_surface_sampling(mesh, num_points, noise_std, hash_resolution, contain_method):
...
def _balanced_sampling(mesh, num_points, sampler_fn, sampler_args):
    ...
    for _ in range(max_iterations):
        ...
        inside_points[inside_count:inside_count + points_to_add] = new_inside[:points_to_add]
        inside_count += points_to_add
            
        # Fill outside buffer
        new_outside = batch_points[~batch_occs]
        points_to_add = min(len(new_outside), target - outside_count)
        outside_points[outside_count:outside_count + points_to_add] = new_outside[:points_to_add]
    ...
    if inside_count != target or outside_count != target:
        ...
        if inside_count < target:
            ...
            pad_indices = torch.randint(0, inside_count, (repeat_count,), device=mesh.device)
        ...
        if outside_count < target:
            ...
            pad_indices = torch.randint(0, outside_count, (repeat_count,), device=mesh.device)
...
@torch.inference_mode()
def sample_volume_sphere(mesh, num_points, hash_resolution=512, contain_method="occnets", get_occ=True, balance=False):
    """
    ...
    if not balance:
        return _unbalanced_volume_sampling(mesh, num_points, radius, hash_resolution, contain_method, get_occ)
    
    sampler_args = (radius, hash_resolution, contain_method, True)  # Always use get_occ=True for balanced
    result = _balanced_sampling(mesh, num_points, _unbalanced_volume_sampling, sampler_args)
...
@torch.inference_mode()
def sample_near_surface(mesh, num_points, noise_std=0.05, hash_resolution=512, contain_method="occnets", get_occ=True,  balance=False):
    """
    ...
    if not balance:
        return _unbalanced_surface_sampling(mesh, num_points, noise_std, hash_resolution, contain_method)
    
    sampler_args = (noise_std, hash_resolution, contain_method)
    result = _balanced_sampling(mesh, num_points, _unbalanced_surface_sampling, sampler_args)
