"""
Main training script for the PartVAE model.
"""
...
def get_args_parser():
    parser = argparse.ArgumentParser("Part-aware VAE training", add_help=False)
    parser.add_argument(
        "--batch_size",
        default=64,
        type=int,
...
def main(args):
    ...
    # Optionally evaluate the model only
    # if args.eval:
    #     test_stats = evaluate(data_loader_val, model, device)
    #     print(
    #         f"iou of the network on the {len(dataset_val)} test images: {test_stats['iou']:.3f}"
    #     )
    #     exit(0)
