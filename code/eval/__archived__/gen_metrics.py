...
try:
    ...
    print(
        "distChamferCUDA not available; fall back to slower version. You should better make this work."
    )
...
if __name__ == "__main__":
    ...
    for cat_id in range(3, len(categories)):
        ...
        v1_pcs = collect_pcs(
            v1_path, categories[cat_id], number=5000, transform=True, visual=False
        )
        v1_pcs = v1_pcs[:116]
        print(f"loaded {v1_pcs.size(0)} {categories[cat_id]} from v1")

        v2_pcs = collect_pcs(
            v2_path, categories[cat_id], number=5000, transform=False, visual=False
        )
        v2_pcs = v2_pcs[:116]
        print(f"loaded {v1_pcs.size(0)} {categories[cat_id]} from v2")

        gt_pcs = collect_pcs(
            gt_path, categories[cat_id], number=5000, transform=False, visual=False
        )
        gt_pcs = gt_pcs[:116]
        print(f"loaded {v1_pcs.size(0)} {categories[cat_id]} from gt")

        v1_results = compute_all_metrics(
            v1_pcs.cuda(), gt_pcs.cuda(), batch_size=50, accelerated_cd=True
        )
        print(f"v1-{categories[cat_id]}:", v1_results)
        v2_results = compute_all_metrics(
            v2_pcs.cuda(), gt_pcs.cuda(), batch_size=50, accelerated_cd=True
        )
