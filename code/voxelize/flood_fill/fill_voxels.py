...
def get_module(verbose=False):
    ...
    if not _corenet_cpp:
        ...
        if module_path:
            print(f"Loading flood fill routine from {module_path}")
            ...
            print(
                f"JIT compiling flood fill routine from sources... (can take a while on the first run)"
            )
            extra_cuda_flags = os.environ.get("FILL_VOXELS_CUDA_FLAGS", "")
            extra_cuda_flags = list(filter(None, extra_cuda_flags.split(";")))
            p1 = os.path.join(__dir, "fill_voxels_cpu.cc")
            # p2 = os.path.join(__dir, "fill_voxels_gpu.cu")
            p3 = os.path.join(__dir, "module.cc")
            _corenet_cpp = cpp_extension.load(
                name=module_name,
                sources=[str(p1), str(p3)],
                verbose=verbose,
                with_cuda=False,
                # extra_cuda_cflags=(
                #     ["-std=c++17", "-O2", "-Xcudafe=--display_error_number"]
                #     + extra_cuda_flags
                # ),
                extra_cflags=[
                    "-std=c++17",
            ...
            print(f"Finished JIT compiling")
    return _corenet_cpp
